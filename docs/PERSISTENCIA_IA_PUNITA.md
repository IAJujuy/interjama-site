...
La pantalla `ia_punita.html` conserva el flujo visual actual, pero cambia una regla critica: el boton **enviar formulario** solo abre WhatsApp despues de que la API confirme que la ficha fue persistida en PostgreSQL.
...
POST /api/consultas
        ↓
La API responde ok:true despues de insertar en PostgreSQL
        ↓
...
En `ia_punita.html` ahora hay resolucion automatica de API:

```js
const PRODUCTION_PERSISTENCE_API_URL = "https://api.interjama.com/api/consultas";
const LOCAL_PERSISTENCE_API_URL = "http://localhost:3000/api/consultas";
```

- En local, archivo suelto, `localhost` o `127.0.0.1`, usa `http://localhost:3000/api/consultas`.
- En dominio real, usa `https://api.interjama.com/api/consultas`.

Tambien se puede forzar manualmente desde consola del navegador:

```js
localStorage.setItem("INTERJAMA_API_URL", "http://localhost:3000/api/consultas");
```

Para volver al comportamiento automatico:

```js
localStorage.removeItem("INTERJAMA_API_URL");
```
...
En esta version, si la API o la base fallan, WhatsApp no se abre automaticamente. La regla operativa es estricta:

```txt
Sin persistencia confirmada -> no se abre WhatsApp.
```

Esto protege la trazabilidad: cada WhatsApp enviado debe tener una ficha previa en PostgreSQL.
